...
\end{titlepage}
    ...
    \begin{wrapfigure}[12]{l}{0.4\linewidth}
        \centering
    ...
    \subsubsubsection{Draughts}
    For draughts, we will need only one object, an environment. The environment will be used for training the reinforcement learning agent as well
    ...
    well as functions to compute the available moves and check for a win. An example of UML for this environment can be seen below. Including functions
    for the user as well as the agent to interact with the environment.
    ...
    \begin{wrapfigure}[22]{l}{0.5\linewidth}
        \centering
        \includegraphics[scale=0.4]{AutoDiffUML.drawio.png}
        \caption{Pre-design UML for Auto Diff}
    \end{wrapfigure}
    ...
    attrributes, such as its gradient and if it's gradient requires computing. If the tensor does require a gradient then it will also store its parent
    tensors, which will be used for the backward DFS of our computational graph.
    ...
    Tensor operations will all have a backward and forward method - inherited from the function class. These functions will represent operations such as those seen in the
    diagram (Addition, Division, Log, etc.). The operations such as sum and mean, that operate on the tensor itself will also be represented by a function object.
    ...
    a search of available actions to train the network towards - composed of a node class to make up this tree and an agent class which abstracts the processes of the previous classes. \\
    ...
    \subsubsection{Mathematical Forumla}
    \subsubsubsection{Automatic Differentiation}
