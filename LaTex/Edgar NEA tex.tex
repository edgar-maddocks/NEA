...
\begin{document}
    ...
    class AlphaLoss(Module):
        ...
        def forward(
            ...
            val_sqe = (true_value - predicted_value) ** 2 # MSE half
            pol_bcel = mcts_pol.T() @ net_pol.log() # BCEL half
    ...
    derivatives for each operation by hand and then checking the module produces these values inside of a function.
    ...
    VIDEO 1
    Show modularity - create a model with a 2d conv layer + relu - reshape - dense layer + sigmoid.
    Pass some data through this model and show the output.
    Save the model
