...
\end{titlepage}
    ...
    To understand the process of removing broadcasted dimensions, we must first understand why numpy uses broadcasting.  
    Given we want to multiply a matrix (with size $3 \times 1$) by a scalar (of $2$) we can complete this operation by iterating over each element in our matrix and
    multipling it by our scalar. At a larger scale this becomes quite inefficient - especially when it comes to multiplying matrices with each
    other. To reduce this, the numpy library would broadcast our scalar to a matching $3 \times 1$ matrix and complete a vector multiplication.
    Using a different example to understand how to remove any broadcasted dimensions, say we have a matrix $a$, and its shape is $3 \times 1 \times 4$ and its initially computed
    gradient ($da$) has shape $5 \times 3 \times 1 \times 4$ after any broadcasting. We must first sum out any added dimensions using the algorithm below.
    ...
    \pagebreak
    \vspace*{5mm}
    Cross-correlation of 2D matrices has been mentioned and demonstrated in the analysis (\myhy{Analysis-MathematicalFormulae}{Mathematical Formulae}), but in the design I will provide a visual representation of
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.35]{Cross-Correlation-Algorithm.drawio.png}
